    ...
    if(len(BL_ACK)):
        ...
        if(Ack_intValue == 0xCD):
            ...
            if(Command_Code == CBL_GET_VER_CMD):
                ...
                Process_CBL_MEM_WRITE_CMD(received_Array)
...
def Process_CBL_GET_VER_CMD(received_Array):
    print("\n   Bootloader Vendor ID : ", received_Array[2])
    print("   Bootloader Version   : ", received_Array[3], ".", received_Array[4], ".", received_Array[5])

def Process_CBL_GET_HELP_CMD(received_Array):
    print("\n   Supported Commands : ", end = ' ')
    for command in received_Array[2:]:
...
def Process_CBL_MEM_WRITE_CMD(received_Array):
    global Memory_Write_All
    BL_Write_Status = 0
    # Serial_Data = Read_TCP(Data_Len)
    # BL_Write_Status = bytearray(Serial_Data)
    if(received_Array[2] == FLASH_PAYLOAD_WRITE_FAILED):
        print("\n   Write Status -> Write Failed or Invalid Address ")
    elif (received_Array[2] == FLASH_PAYLOAD_WRITE_PASSED):
...
def Decode_CBL_Command(Command):
    ...
    if(Command == 1):
        ...
        while(BinFileRemainingBytes):
            ...
            print("BL_Host_Buffer[7 + BinFileReadLength]: ", BL_Host_Buffer[7 + BinFileReadLength])
            print("BL_Host_Buffer[8 + BinFileReadLength]: ", BL_Host_Buffer[8 + BinFileReadLength])
            print("BL_Host_Buffer[9 + BinFileReadLength]: ", BL_Host_Buffer[9 + BinFileReadLength])
            print("BL_Host_Buffer[10 + BinFileReadLength]: ", BL_Host_Buffer[10 + BinFileReadLength])
